from crosscurve_arbitrage import get_all_routes, get_profitable_route
...
def account_menu() -> list:
    ...
    while not choice:
        ...
        if choice == '1':
            ...
            all_routes = get_all_routes(filtered_tokens, swap_only_plus, swap_plus_size, max_swap_loss, slippage, amount)
            print("Все роуты, подходящие под запрос")
            for route in all_routes:
                print(f"{route.get("from_chain")} {route.get("from_token")} -> {route.get("to_chain")} {route.get("to_token")} | {route.get("amount_in")} : {route.get("amount_out")}")
            print("---------------------------------------------------------------------")
